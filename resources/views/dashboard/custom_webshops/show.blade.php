@extends('layouts.spaces.dashboard', ['activePage' => 'custom-webshops', 'titlePage' => __('Custom Webshop'), 'activePanel' => 'dashboard'])
    ...
    <div class="container-fluid">
      ...
      <div class="row bg-white project-status-bar">
        ...
        <div class="col-lg-4 col-md-4">
          ...
          <a href="{{ route('custom_webshop.edit', $project->id) }}" class="btn btn-success text-white">@lang('app.change_data')</a>
        </div>
      </div>
    </div>
    <div class="row">
              ...
              <div class="project-timeline my-2">
                ...
                <table class="col-md-12 ml-2 my-4">
                  ...
                  <tbody>
                    ...
                    @endif
                    @if (in_array('webdesign_onboarding', $available_status))
                    ...
                    <tr class="timeline {{ ($project->webdesignOnboarding != null && $project->webdesignOnboarding->sent_email == 1) ? '': 'inactive'}} ">
                      <td><strong>@lang('app.webdesign_onboarding')</strong></td>
                      <td>
                        @if ($project->deadlineBySlug('webdesign_onboarding'))
                        <button class="btn btn-round bg-white text-dark deadline" data-toggle="modal" data-target="#deadlineUpdate" data-value="{{ $project->deadlineBySlug('webdesign_onboarding')->deadline }}" data-id="{{ $project->deadlineBySlug('webdesign_onboarding')->id }}">{{ $project->deadlineBySlug('webdesign_onboarding')->deadline }}</button>    
                        @endif
                      </td>
                      <td>
                        @if ($project->webdesignOnboarding && $project->webdesignOnboarding->status == 1)
                        <a href="{{ route('webdesign_onboarding_pdf', $project->webdesignOnboarding->id) }}" class="text-success font-weight-bold">Download</a>
                        @else
                        <p class="{{ ($project->webdesignOnboarding && $project->webdesignOnboarding->sent_email == 1) ? 'text-success': ''}} font-weight-bold">Pending</p>
                        @endif
                      </td>
                      <td>
                        <div class="btn-group dropleft">
                          <button type="button" class="btn {{ ($project->webdesignOnboarding && $project->webdesignOnboarding->sent_email == 1) ? 'btn-success': ''}} dropdown-toggle btn-round p-0" data-toggle="dropdown" aria-haspopup="true"
                          aria-expanded="false">
                            <span class="material-icons">
                              add
                            </span>
                          ...
                          <div class="dropdown-menu">
                            @if (!$project->deadlineBySlug('webdesign_onboarding'))
                            <a class="dropdown-item" data-toggle="modal" data-target="#addDeadline" data-status-slug="webdesign_onboarding"><strong>@lang('app.set_deadline')</strong></a>
                            @endif
                            <a class="dropdown-item" data-toggle="modal" data-target="#create_todo"><strong>@lang('app.add_to_do')</strong></a>
                            @if ($project->webdesignOnboarding && $project->webdesignOnboarding->sent_email == 1)
                            <a class="dropdown-item" href="#"><strong>@lang('app.add_project')</strong></a>
                            <a class="dropdown-item copy-link" href="{{ route('webdesign_onboarding', ['id' => $project->webdesignOnboarding->id, 'token' => $project->webdesignOnboarding->token]) }}"><strong>@lang('app.copy_form_link')</strong></a>
                            <div class="dropdown-divider"></div>
                            <span class="dropdown-item resend-email" data-status="webdesign_onboarding" data-project="{{ $project->id }}"><strong>@lang('app.resend_email')</strong></span>
                            @endif
                          </div>
                        </div>
                      </td>
...
@include('components.modals.calendar', ['page' => 'custom_webshop'])
      ...
      $('#delete-project').click(function() {
        ...
        $.ajax({
          ...
          success: function(data) {
            toastr.success(data.msg, '@lang("app.success")', toastr_options);
            setTimeout(function() {
              window.location.href = '{{ route("custom_webshop.index") }}'
            }, 2000)
          }
        });
