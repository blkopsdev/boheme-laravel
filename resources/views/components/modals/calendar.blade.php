            ...
            <div class="col-md-8">
              ...
              <div class="form-group">
                <select class="selectpicker form-control no-bg" id="select_project" name="project" data-style="btn btn-success text-white" title="@lang('app.choose_project')">
                  ...
                  <option value="{{ $project->id }}">{{ $project->company_name }} : {{ $project->name }}</option>
                  @endforeach
                </select>
              </div>
              @endif
...
<div class="modal fade" id="create_calendar" tabindex="-1" role="dialog" aria-labelledby="calendarCreateLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title text-success" id="calendarCreateLabel"><strong>@lang('app.add_calendar')</strong></h4>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form action="{{ route('create_calendar') }}" method="post" class="m-0">
          @csrf
          <div class="row">
            <div class="col-md-8">
              <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                <label for="calendar_name" class="relative-label">@lang('app.name')</label>
                <input type="text" name="name" id="calendar_name" class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" required>
                @if ($errors->has('name'))
                  <span id="name-error" class="error text-danger" for="input-name">{{ $errors->first('name') }}</span>
                @endif
              </div>
              <div class="form-group{{ $errors->has('bg_color') ? ' has-danger' : '' }}">
                <label for="bg_color" class="relative-label">@lang('app.pick_bg_color')</label>
                <input type="text" id="bg_color" name="bg_color" class="form-control color-picker no-bg{{ $errors->has('bg_color') ? ' is-invalid' : '' }}" value="{{ auth()->user()->company->main_color }}" required>
                @if ($errors->has('bg_color'))
                  <span id="color-error" class="error text-danger" for="input-color">{{ $errors->first('bg_color') }}</span>
                @endif
              </div>
              <div class="form-group{{ $errors->has('color') ? ' has-danger' : '' }}">
                <label for="color" class="relative-label">@lang('app.pick_txt_color')</label>
                <input type="text" id="color" name="color" class="form-control color-picker no-bg{{ $errors->has('color') ? ' is-invalid' : '' }}" value="{{ auth()->user()->company->other_color }}">
                @if ($errors->has('color'))
...
<div class="modal fade" id="update_calendar" tabindex="-1" role="dialog" aria-labelledby="calendarUpdateLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title text-success" id="calendarUpdateLabel"><strong>@lang('app.edit_calendar')</strong></h4>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form action="{{ route('edit_calendar') }}" method="post" class="m-0">
          @csrf
          <div class="row">
            <div class="col-md-8">
              ...
              <div class="form-group">
                <label for="bg_color" class="relative-label">@lang('app.pick_bg_color')</label>
                <input type="text" name="bg_color" class="form-control color-picker no-bg" required>
              </div>
              <div class="form-group">
                <label for="color" class="relative-label">@lang('app.pick_txt_color')</label>
                <input type="text" name="color" class="form-control color-picker no-bg">
              </div>
            </div>
          </div>
          <div class="row border-top mt-3">
